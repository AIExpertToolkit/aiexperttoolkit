# deploy_consulting.ps1  — adds Consulting page + nav, commits, pushes, opens PR
...
$branch = 'feat-consulting-sprint'
& git fetch --all --prune
& git checkout -B $branch origin/main 2>$null; if($LASTEXITCODE -ne 0){ & git checkout -B $branch }

# 1) Ensure "Consulting" link in header
$header = '_includes/header.html'
if (Test-Path $header) {
  $s = Get-Content -Raw $header
  if ($s -notmatch '/consulting\.html') {
    $inject = '      <a href="{{ site.baseurl }}/consulting.html" {% if page.url contains ''/consulting'' %}aria-current="page"{% endif %} class="nav-accent">Consulting</a>' + [Environment]::NewLine + '    </nav>'
    $s = [regex]::Replace($s, '</nav>', $inject, 1)
    Set-Content -Encoding utf8 -NoNewline $header $s
  }
}

# 2) Append pricing/CRO CSS once
$css = 'assets/css/base.css'
if (Test-Path $css) {
  $cs = Get-Content -Raw $css
  if ($cs -notmatch '/\* ── Pricing') {
@"
...
"@ | Add-Content -Encoding utf8 -Path $css
  }
}

# 3) Write consulting.html (full page)
$consult = @'
---
layout: default
...
Set-Content -Encoding utf8 'consulting.html' $consult

# Commit & push
& git add -A
$staged = & git diff --cached --name-only
if ($staged) { & git commit -m "feat(service): add Build-With-You Website Sprint page; nav link; pricing CSS; GA4 event" }
